...
export const db = {
  ...
  // Cliente operations
  cliente: {
    async findFirst(args: { where: { negocioId_email?: { negocioId: string; email: string }; qrCodigo?: string; email?: string; activo?: boolean; negocioId?: string }; include?: { negocio?: { select: { nombre: boolean } }; _count?: { select: { compras: boolean } }; compras?: { orderBy: { fecha: string }; take: number; select: { fecha: boolean; compraNumero: boolean } } } }) {
      const db = getClient()
